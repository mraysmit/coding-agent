...
#### 3.2.1 Index Build Process
The example index is a **checked-in JSON file** (`knowledge/example-index.json`) built by a one-time build script and updated when the APEX example corpus changes.

**Build steps (run manually or via Maven profile):**
1. Scan all `.yaml` files in the three corpus directories.
2. For each file, parse YAML and extract:
   - `metadata.type` → `docType`
   - top-level section keys → `features` (e.g., `["rules", "enrichments", "rule-groups"]`)
   - all `#`-prefixed tokens in `condition`/`expression`/`calculation` fields → `spELPatterns`
   - presence of `data-source-refs`, `component-refs`, `config-file` → `crossFileRefs: true/false`
   - rule/enrichment count → `complexity` (low/medium/high)
3. Write to `knowledge/example-index.json` as an array of objects.
4. Commit alongside the syntax artifact.

**Index entry schema:**
```json
{
  "path": "apex-playground/examples/enrichment/calculation-enrichment-test.yaml",
  "docType": "rule-config",
  "features": ["enrichments", "calculation-enrichment"],
  "spELPatterns": ["#quantity * #price"],
  "crossFileRefs": false,
  "complexity": "low"
}
```

**Runtime retrieval:** `ApexExampleRetrievalTool.searchExamples()` loads the index, filters by `docType` + `features` intersection, ranks by feature overlap count, returns top-K paths. The agent then reads the matched files via `FileSystemTools`.
...
#### 3.3.1 Artifact Lifecycle
**Author:** Initially auto-generated by a one-time script that extracts structure from `APEX_YAML_REFERENCE.md` (the keyword table, type-specific required fields from `ApexYamlLexicalValidator`, and canonical templates from playground examples). Then **hand-reviewed and committed** as a versioned project file.

**Update trigger:** When APEX docs or `ApexYamlLexicalValidator` grammar constants change, re-run the extraction script and diff against the committed version.

**Injection point:** Loaded at startup and prepended to the APEX-specific system prompt section. Target size: < 3,000 tokens to leave room for few-shot examples and user context.

**Source of truth for required fields:** `ApexYamlLexicalValidator` defines `REQUIRED_METADATA_FIELDS`, `VALID_DOCUMENT_TYPES`, `TYPE_SPECIFIC_REQUIRED_FIELDS`, and `TYPE_REQUIRED_SECTIONS` as static constants. The syntax artifact must mirror these exactly.
...
3. `ApexCompileTool`
     - `validateLexical(yamlContent)` — wraps `ApexYamlLexicalValidator.validateFile()`
       ...
       - Must write YAML content to a temp file since `validateFile()` takes a `Path`
     - `validateCompiler(yamlContent)` — wraps `ApexYamlCompiler.compile()`
       - Returns `CompilationResult` with `success`, `message`, `generatedCode`
       - Note: compiler currently has TODOs for some analysis phases; lexical validator is more thorough
     - Both methods return structured JSON with error classification (see Section 6)
...
5. `ApexExpectationTool`
     - `assertExpectedOutcomes(actualResultJson, expectationsJson)`
     ...
     **Expectation schema:**
     ```json
     {
       "expectations": [
         {
           "type": "RULE_MATCH",
           "ruleId": "high-value-check",
           "shouldMatch": true,
           "expectedSeverity": "ERROR"
         },
         {
           "type": "RULE_MATCH",
           "ruleId": "currency-check",
           "shouldMatch": false
         },
         {
           "type": "ENRICHED_FIELD",
     ...
     **Supported expectation types:**
     - `RULE_MATCH` — assert a specific rule matched or didn't, optionally check severity
     - `ENRICHED_FIELD` — assert a field exists in `enrichedData` with a value condition (`EQUALS`, `GREATER_THAN`, `LESS_THAN`, `NOT_NULL`, `CONTAINS`)
     ...
     **Who writes expectations?** The Author Agent generates them alongside the YAML and test data. The Planner Agent may also emit high-level expectations from the requirements. The LLM constructs the JSON; the tool evaluates it deterministically.
...
### Phase 0: Foundation
- add APEX dependencies to `pom.xml`
- smoke test compiler and runtime invocation from a JUnit test
- verify Spring Boot 4.x / APEX 3.4.x compatibility (see Section 10)
- **create the condensed syntax artifact** (`knowledge/apex-syntax-compact.yaml`) by extracting from `ApexYamlLexicalValidator` constants + `APEX_YAML_REFERENCE.md` keyword table
- **build the example index** (`knowledge/example-index.json`) by scanning the three corpus directories
